...
import { Brand, CarDetailData, CarModel, Category, Color, CountryObject, Feature, FuelType, Image, Seller, Version } from '@/lib/object';
...
const CreateCarForm = ({ brands, carModels, categories, versions, features, fuelTypes, sellers, colors, countries, car }: CreateFormProps) => {
    ...
    useEffect(() => {
        ...
        if (
            !car &&
            data.car_model_id &&
            !carModels.some((model) => model.id.toString() === data.car_model_id && model.brand_id.toString() === selectedBrandId)
        ) {
    ...
    useEffect(() => {
        ...
        if (
            !car &&
            data.version_id &&
            !versions.some((version) => version.id.toString() === data.version_id && version.car_model_id.toString() === selectedCarModelId)
        ) {
            setData('version_id', '');
        }
    ...
    const onDropMultipleImages = (acceptedFiles: File[]) => {
        ...
        const newValidFiles = acceptedFiles.filter(
            (file) => validTypes.includes(file.type) && !data.images.some((f) => f.name === file.name && f.size === file.size), // Prevent duplicates of new files
        );
    ...
    const {
        getRootProps: getMultipleImagesRootProps,
        getInputProps: getMultipleImagesInputProps,
        isDragActive: isMultipleImagesDragActive,
    } = useDropzone({
    ...
    return (
        <div className="px- container mx-auto py-10">
            <form onSubmit={onSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-x-3 sm:grid-cols-4">
                            ...
                            <SelectContent>
                                {brands.length > 0 ? (
                                    ...
                                    <SelectItem value="placeholder" disabled>
                                        {' '}
                                        {/* Empty string for disabled placeholder */}
                                        No brands available
                                    </SelectItem>
                                )}
                            </SelectContent>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={(value) => setData('fuel_type_id', value)} value={data.fuel_type_id}>
                ...
                <div className="grid grid-cols-1 gap-x-3 sm:grid-cols-4">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={(value) => setData('status', value as 'right' | 'left')} value={data.status || ''}>
                ...
                <div className="grid grid-cols-1 gap-x-3 sm:grid-cols-4">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={(value) => setData('interior_color_id', value)} value={data.interior_color_id}>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={(value) => setData('exterior_color_id', value)} value={data.exterior_color_id}>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={(value) => setData('wheel_driver', value as 'fwd' | 'rwd' | 'awd')} value={data.wheel_driver}>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={(value) => setData('seller_id', value)} value={data.seller_id}>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={(value) => setData('origin_country_id', value)} value={data.origin_country_id}>
                ...
                <div className="grid grid-cols-1 gap-x-3 sm:grid-cols-4">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input
                            ...
                            disabled={processing}
                        />
                            <InputError message={errors['dimensions.width_mm' as keyof typeof errors]} className="mt-2" />
                    </div>
                    <div className="grid gap-2">
                        ...
                        <Input
                            ...
                            disabled={processing}
                        />
                        <InputError message={errors['dimensions.lenght_mm' as keyof typeof errors]} className="mt-2" />
                    </div>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input
                            ...
                            disabled={processing}
                        />
                        <InputError message={errors['dimensions.height_mm' as keyof typeof errors]} className="mt-2" />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-x-3 sm:grid-cols-4">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={(value) => setData('discount_type', value as 'amount' | 'percent')} value={data.discount_type}>
                    ...
                    <div className="grid gap-2">
                        <Label htmlFor="discount">Discount {data.discount_type === 'percent' ? '(%)' : '($)'}</Label>
                        <Input
                            id="discount"
                ...
                <div>
                    <div className="mb-4">
                        ...
                        <p className="text-muted-foreground text-sm">Select car options.</p>
                    </div>
                    <div className="grid grid-cols-1 gap-x-3 sm:grid-cols-4">
                        {features.length > 0 ? (
                                    ...
                                    <Checkbox
                                        ...
                                        onCheckedChange={(checked) => {
                                            if (checked) {
                                                ...
                                                setData(
                                                    'features',
                                                    data.features.filter((f) => f.id !== feature.id),
                                                );
                                            }
                                        }}
                                    />
                                    <Label htmlFor={feature.feature_name} className="cursor-pointer text-sm font-normal">
                ...
                <div className="grid gap-2">
                    ...
                    <div
                        ...
                        className={`flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-6 ${isMainImageDragActive ? 'border-primary' : 'border-gray-300'} ${errors.image ? 'border-destructive' : ''} `}
                    >
                        ...
                        <p className="mt-1 text-xs text-gray-500">(PNG, JPG, JPEG up to 1MB)</p>
                    </div>
                    <p className="text-muted-foreground text-sm">Upload a single JPEG/PNG image for the main car image.</p>
                    <InputError message={errors.image} className="mt-2" />
                    {mainImagePreview && (
                        <div className="mt-2">
                            <img src={mainImagePreview} alt="Main Image Preview" className="h-40 w-auto rounded object-contain shadow" />
                        </div>
                    )}
                </div>
                ...
                <div className="grid gap-2">
                    ...
                    <div
                        ...
                        className={`flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-6 ${isMultipleImagesDragActive ? 'border-primary' : 'border-gray-300'} ${errors.images ? 'border-destructive' : ''} `}
                    >
                    ...
                    <p className="text-muted-foreground text-sm">Upload JPEG/PNG images only. Max 6 images.</p>
                    <InputError message={errors.images} className="mt-2" />
                    {allImagesForPreviews.length > 0 && (
                        <div className="mt-2 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                            {allImagesForPreviews.map(
                                (
                                    item, // Iterate over ImagePreviewItem
                                ) => (
                                    <div key={item.id} className="group relative">
                                        <img src={item.src} alt={`Preview`} className="h-40 w-full rounded object-cover shadow" />
                                        <button
                                            type="button"
                                            onClick={() => removeImage(item)} // Pass the entire item to removeImage
                                            className="absolute top-1 right-1 rounded bg-red-600 px-2 py-1 text-xs text-white opacity-80 hover:opacity-100"
                                        >
                                            Remove
                                        </button>
                                    </div>
